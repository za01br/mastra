Here's my suggested revision of the multi-agent workflow document:

# Multi-Agent Workflow

This example demonstrates how to create an Agentic workflow utilizing multiple agents.
...
Create the copywriter agent that will generate the initial blog post, then define the copywriter step that executes the agent and handles the response:
...
Set up the editor agent to refine the copywriter's content, then define the editor step that processes the copywriter's output:
...
Finally, configure the workflow and execute the steps:
