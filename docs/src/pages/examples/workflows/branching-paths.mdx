...
const stepTwo = new Step({
  id: "stepTwo",
  execute: async ({ context: { machineContext } }) => ({ 
    isDivisibleByFive: machineContext.stepResults.stepOne.result.doubledValue % 5 === 0
  })
});
...
const stepFour = new Step({
  id: "stepFour",
  execute: async ({ context: { machineContext } }) => ({ 
    isDivisibleByThree: machineContext.stepResults.stepThree.result.incrementedValue % 3 === 0
  })
});
