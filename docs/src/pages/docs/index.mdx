...
* **[Model routing](/docs/llm-models/00-overview.mdx)**: With Mastra, you can use the same syntax to call any LLM provider in Mastra, including OpenAI, Anthropic, and Google Gemini.
...
* **[Evals](/docs/08-running-evals.mdx)**: Mastra provides automated evaluation metrics that use model-graded, rule-based, and statistical methods to assess LLM outputs, with built-in metrics for toxicity, bias, relevance, and factual accuracy. You can also define your own evals.
