  ...
  execute: async ({ context, mastra }) => {
    ...
    const prompt = `
      ...
      Return valid JSON with fields:
      {
        ...
        "isTechnical": true or false,
        "specialty": "candidate’s specialty (if technical)",
        "resumeText": "full text used for reference"
      }
    `;
...
const askAboutSpecialty = new Step({
  ...
  execute: async ({ context, mastra }) => {
    ...
    const prompt = `
      ...
      for ${context.candidateName} about how they got into "${context.specialty}".
...
const askAboutRole = new Step({
  ...
  execute: async ({ context, mastra }) => {
    ...
    const prompt = `
      You are a recruiter. Given the resume below, craft a short question
      for ${context.candidateName} asking what interests them most about this role.
      Resume: ${context.resumeText}
    `;
    const res = await llm.generate(prompt);
    return { question: res?.text?.trim() || '' };
...
## 5. Define and Execute the Workflow

You now combine the steps to implement branching logic based on the candidate’s technical status. The workflow first gathers candidate data, then either asks about their specialty or about their role, depending on isTechnical. The code chains gatherCandidateInfo with askAboutSpecialty and askAboutRole, commits the workflow, and executes it.
