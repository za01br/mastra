...
The `DifferenceMetric` class measures the textual differences between two strings using sequence matching. It provides detailed information about changes, including the number of operations needed to transform one text into another.

## Basic Usage

```typescript
import { DifferenceMetric } from "@mastra/evals";
