...
```ts showLineNumbers copy
import { embedMany } from "@mastra/core/embeddings/openai";
...
```ts showLineNumbers copy
import { embed } from '@mastra/core/embeddings/cohere';
...
```ts showLineNumbers copy
import { embedMany as openAIEmbedMany } from "@mastra/core/embeddings/openai";
import { embedMany as cohereEmbedMany } from "@mastra/core/embeddings/cohere";
...
const { embeddings: openAIEmbeddings } = await openAIEmbedMany(chunks, {
  model: "text-embedding-3-small",
});
...
const { embeddings: cohereEmbeddings } = await cohereEmbedMany(chunks, {
  model: "embed-english-v3.0",
});
