...
const audio = await weatherAgent.speak('Hello, World!');
const filePath = path.join(process.cwd(), 'weather-agent.mp3');
...
const audioFilePath = path.join(process.cwd(), '/weather-agent.mp3');
const audioStream = createReadStream(audioFilePath);

try {
  console.log('Transcribing audio file...');
  const transcription = await weatherAgent.listen(audioStream);
