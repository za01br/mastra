...
Prompt templates in Mastra help maintain consistency and leverage proven prompting techniques. The `createPrompt` function provides a builder pattern for creating effective and reusable prompts.
...
```typescript
createPrompt("Explain this code", {
...
const template = createPrompt<CodeReviewVars>("Review code changes")
  .text("Review {{changes}} changes in {{file}}")
  .constraints([
    "Focus on {{file}} specific patterns",
    "Suggest maximum {{changes}} improvements",
  ]);

// Use the template with different values
const review1 = template.toString({
  file: "auth.ts",
  changes: 3,
});

const review2 = template.toString({
  file: "api.ts",
  changes: 5,
});
```
...
// Use the template
const report = bugReportTemplate.toString({
  severity: "high",
  component: "checkout",
});

const report = bugReportTemplate.toMessage({
  severity: "high",
  component: "checkout",
});
```
