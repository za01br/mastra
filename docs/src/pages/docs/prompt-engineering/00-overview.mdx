...
const bugClassifier = createPrompt("Classify the bug severity")
  .examples([
    {
...
const codeReviewer = createPrompt("Review this code")
  .context(
    `
...
const translator = createPrompt("Translate this text to French").text(
  "Hello, how are you?",
);
```
...
const storyWriter = createPrompt("Convert these requirements into user stories")
  .examples([
    {
...
const debugger = createPrompt("Debug this performance issue")
  .text("The API response time increased from 200ms to 2s after the last deployment")
  .thinking({
...
const systemDesigner = createPrompt(
...
const queryOptimizer = createPrompt("Optimize this database query")
...
const architectureReviewer = createPrompt("Review this architecture proposal", {
