...
**[RAG](https://mastra.ai/docs/guide/how-to/03-knowledge-sources)** | Retrieval-augemented generation (RAG) lets you construct a knowledge base for agents. RAG is an ETL pipeline with specific querying techniques. You “chunk” documents into smaller pieces, “embed” each chunk (transform it to a vector), and load it into a vector DB. Then, you embed your query text into a vector, search the DB for similar vectors, and feed the results into an LLM call.
**[Integrations & Syncs](https://mastra.ai/docs/guide/how-to/06-adding-integrations)** | Mastra provides helpers for you to load your data into a RAG pipeline: scraping from websites, loading from files, or syncing from third-party SaaS services. It also provides third-party APIs you can give to an agent to take action. 
**[Evals](https://mastra.ai/docs/guide/how-to/05-running-evals)** | Evals are how you test agents and workflows. You can test intermediate steps of a workflow, or user-facing LLM results. You can check accuracy, relevance, factual consistency, length adherence, and more. You might write evals checking semantic distance, search for a particular string, or have another LLM evaluate your LLM's response. 
--------|--------
