...
```bash copy npm2yarn
npm init -y
npm install typescript tsx @types/node @mastra/core --save-dev
```
...
async function main() {
  const agent = new Agent({
    maxSteps: 3,
    model: { type: 'anthropic' }, // or 'openai' if you prefer
    systemPrompt: `You are a helpful assistant who writes creative stories.`,
    tools: {},
  });
