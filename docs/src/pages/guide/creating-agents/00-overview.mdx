...
Agents can return structured data by providing a schema:
...
Certainly! Based on your request and considering the previous discussions and updates to the Mastra API, I've made several changes to the **Agents Overview** document to ensure it's up-to-date, accurate, and clear. Below is the revised document with the proposed changes.
...
```typescript:docs/src/pages/guide/creating-agents/00-overview.mdx
import { Agent } from "@mastra/core";
...
console.log("Agent:", response.text);
```docs/src/pages/guide/creating-agents/00-overview.mdx

### **Streaming Responses**

For real-time applications, you can stream the agent's response:
