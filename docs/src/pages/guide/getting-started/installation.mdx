...
Install the `@mastra/core` package:
...
1. **Create a `mastra` directory inside your `src` directory:**

```bash copy
mkdir src/mastra
```

2. **Create an `agents` directory inside `src/mastra`:**

```bash copy
mkdir src/mastra/agents
```

3. **Create the `story-writer` agent file:**

```bash copy
touch src/mastra/agents/story-writer.ts
```

Add the following code to `src/mastra/agents/story-writer.ts`:

```typescript:src/mastra/agents/story-writer.ts showLineNumbers
...
4. **Create the Mastra entry point in `src/mastra/index.ts`:**

```typescript:src/mastra/index.ts showLineNumbers
...
### (Optional) Update `src/index.ts` to Test the Agent

You can update `src/index.ts` to test your agent directly:
...
You can run the script to test that everything is set up correctly:
