...
By default, Mastra emits logs locally to the `/logs` directory for viewing in the admin console.You can view logs in the admin console under the `Logs` tab.

We're working on getting Mastra to support viewing logs in context in the chat UI (so you can see which tool calls are being made) and in the workflow UI (so you can see the history of a workflow run).

## Viewing logs in the admin console

![Mastra logs](../../public/log.png)
