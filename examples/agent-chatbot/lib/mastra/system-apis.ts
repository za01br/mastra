...
export async function getTeams() {
    ...
    return data.sports?.[0].leagues?.[0].teams.map(({ team }: { team: { id: string, displayName: string } }) => {
        return {
            id: team.id,
        ...
        return {
            ...
            competitions: e.competitions.map((c: Record<string, any>) => {
                return {
                    ...
                    headlines: c.headlines?.map((h: { description: string, shortLinkText: string }) => {
                        return {
...
export async function getScores({ week, day }: { week: string; day?: string }) {
    ...
    if (day) {
        return res[day].filter((e: { week: number }) => e.week === parseInt(week, 10))
    }
