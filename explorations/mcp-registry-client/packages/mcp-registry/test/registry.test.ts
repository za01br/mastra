...
describe('RegistryClient', () => {
  ...
  describe('constructor', () => {
    it('should throw if no options provided', () => {
      ...
      expect(() => new RegistryClient()).toThrow('RegistryClient: URL is required')
    })
    ...
    it('should throw if URL is empty', () => {
      expect(() => new RegistryClient({ url: '' })).toThrow('RegistryClient: URL is required')
    })
  })
  ...
  describe('connect', () => {
    it('should connect to mock registry', async () => {
      ...
      expect(directory.description).toBe('Discover and use MCP servers for your AI applications')
    ...
    it('should throw for unknown registry URL', async () => {
      ...
      await expect(client.connect()).rejects.toThrow('RegistryClient: Registry not found at https://unknown.com')
    })
  })
  ...
  describe('getServerDefinition', () => {
    it('should throw if no input provided', async () => {
      // @ts-expect-error Testing invalid input
      await expect(registry.getServerDefinition()).rejects.toThrow('RegistryClient: Server ID is required')
    })

    it('should throw if no server ID is provided', async () => {
      await expect(registry.getServerDefinition({ id: '' })).rejects.toThrow('RegistryClient: Server ID is required')
    })
    ...
    it('should throw for unknown server ID', async () => {
      ...
      ).rejects.toThrow('RegistryClient: Server "unknown" not found in registry at')
    })
