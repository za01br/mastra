...
describe('Postgres Engine', () => {
    ...
    describe('Connection and Credential Management', () => {
        ...
        it('getConnection', async () => {
            const connection = await engine.getConnection({ connectionId: testConnection.connectionId, name: testConnection.name });
        ...
        it('getConnectionsBySubscriptionId', async () => {
            ...
            if (testConnection.subscriptionId) {
                const connections = await engine.getConnectionsBySubscriptionId({ subscriptionId: testConnection.subscriptionId });
                expect(connections).toBeDefined();
                expect(connections.length).toBeGreaterThan(0);
            }
        })
    ...
    describe('Entity Management', () => {
        it('should create and retrieve an entity', async () => {
            testEntity = await engine.createEntity({ connectionId: testConnection.connectionId, type: 'TEST', kId: testConnection.id });
    ...
    describe('Records', () => {
        it('mergeExternalRecordsForEntity', async () => {
            ...
            const recordsRetrieved = await engine.getEntityRecordsByConnectionAndType({ kId: testConnection.id, type: 'TEST' });
        ...
        it('getRecordByPropertyNameAndValue', async () => {
            const record = await engine.getRecordByPropertyNameAndValue({ type: 'TEST', connectionId: testConnection.connectionId, propertyName: 'name', propertyValue: 'test-record-1' });
            expect(record).toBeDefined();
            expect(record?.data?.name).toBe('test-record-1');
        })

        it('getRecordByPropertyNameAndValues', async () => {
            const records = await engine.getRecordByPropertyNameAndValues({ connectionId: testConnection.connectionId, propertyName: 'name', propertyValues: ['test-record-1', 'test-record-2'] });
            expect(records).toBeDefined();
        ...
        it('syncData', async () => {
            ...
            const recordsRetrieved = await engine.getEntityRecordsByConnectionAndType({ kId: testConnection.id, type: 'FOO' });
