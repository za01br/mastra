  ...
  function recurseDip(obj: Record<string, any>, path: string[]) {
    for (const [key, value] of Object.entries(obj)) {
      ...
      if (value && value.constructor === Object) {
        const hasOnlyEndKeys = endKeys?.length
          ? endKeys.every((v) => Object.keys(value).includes(v))
          : false;
        if (hasOnlyEndKeys) {
        ...
        const contructValue = value.reduce(
          (a, b) => ({ ...a, [value.indexOf(b)]: b }),
          {}
        );
 ...
 */
export function mergeWithDefinedOnly<T extends AnyObject, U extends AnyObject>(
  base: T,
  overrides: U
): T & U {
