...
import { getPricesResponseSchema, getPricesPriceResponseSchema } from './sync-fixtures/stripe';
import { reposGetResponseSchema, reposListForOrgResponseSchema } from './sync-fixtures/github';
...
describe('Factory', () => {
    ...
    describe('GITHUB', () => {
        it.only('should fields for multi data', () => {
            const props = getResponseDataKey({ responseSchema: reposListForOrgResponseSchema })
            console.log(props)
        })
