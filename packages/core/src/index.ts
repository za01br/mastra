    ...
    async connectedPlugins({ context }: { context: { connectionId: string } }) {
        ...
        const connectionChecks = await Promise.all(
            plugins.map(async ({ plugin }) => {
                const connection = await this.dataLayer.getConnectionById({ connectionId: context.connectionId, name: plugin.name });
                return { plugin, connected: !!connection };
            }),
        );
        return connectionChecks.filter(({ connected }) => connected).map(({ plugin }) => plugin);
    }
