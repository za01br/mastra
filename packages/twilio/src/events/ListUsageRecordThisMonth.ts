...
import { API_V2010_ACCOUNT_USAGE_USAGE_RECORD_USAGE_RECORD_THIS_MONTHFields } from '../constants';

import { TwilioIntegration } from '..';

export const ListUsageRecordThisMonth: EventHandler<TwilioIntegration> = ({
  eventKey,
  integrationInstance: { name, dataLayer, getApiClient, config },
  makeWebhookUrl,
}) => ({
  id: `${name}-sync-API_V2010_ACCOUNT_USAGE_USAGE_RECORD_USAGE_RECORD_THIS_MONTH-ListUsageRecordThisMonth`,
  event: eventKey,
  executor: async ({ event, step }: any) => {
    ...
    const records = d?.['usage_records']?.map(r => {
      return {
        ...
        data: r,
        entityType: 'API_V2010_ACCOUNT_USAGE_USAGE_RECORD_USAGE_RECORD_THIS_MONTH',
      };
    });
