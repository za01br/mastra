    ...
    export const ListSipAuthCallsIpAccessControlListMapping: EventHandler<TwilioIntegration> = ({
      eventKey,
      integrationInstance: { name, dataLayer, getApiClient },
      makeWebhookUrl,
    }) => ({
        id: `${name}-sync-API_V2010_ACCOUNT_SIP_SIP_DOMAIN_SIP_AUTH_SIP_AUTH_CALLS_SIP_AUTH_CALLS_IP_ACCESS_CONTROL_LIST_MAPPING-ListSipAuthCallsIpAccessControlListMapping`,
        event: eventKey,
        executor: async ({ event, step }: any) => {
          ...
          // @ts-ignore
          const response = await proxy['/2010-04-01/Accounts/{AccountSid}/SIP/Domains/{DomainSid}/Auth/Calls/IpAccessControlListMappings.json'].get({
          ...
          if (!response.ok) {
            ...
            console.log("error in fetching ListSipAuthCallsIpAccessControlListMapping", JSON.stringify(error, null, 2));
            return
          }        
