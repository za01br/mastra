    ...
    export const ListIncomingPhoneNumber: EventHandler<TwilioIntegration> = ({
      eventKey,
      integrationInstance: { name, dataLayer, getApiClient },
      makeWebhookUrl,
    }) => ({
        id: `${name}-sync-API_V2010_ACCOUNT_INCOMING_PHONE_NUMBER-ListIncomingPhoneNumber`,
        event: eventKey,
        executor: async ({ event, step }: any) => {
          ...
          // @ts-ignore
          const response = await proxy['/2010-04-01/Accounts/{AccountSid}/IncomingPhoneNumbers.json'].get({
            params: { AccountSid },
          ...
          if (!response.ok) {
            ...
            console.log("error in fetching ListIncomingPhoneNumber", JSON.stringify(error, null, 2));
            return
          }        
