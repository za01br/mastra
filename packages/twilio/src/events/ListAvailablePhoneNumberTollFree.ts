    ...
    export const ListAvailablePhoneNumberTollFree: EventHandler<TwilioIntegration> = ({
      ...
      makeWebhookUrl,
        ...
        executor: async ({ event, step }: any) => {
          ...
          // @ts-ignore
          const response = await proxy['/2010-04-01/Accounts/{AccountSid}/AvailablePhoneNumbers/{CountryCode}/TollFree.json'].get({
          ...
          const records = d?.['available_phone_numbers']?.map((r) => {
            return {
              ...
              record: r,
              entityType: API_V2010_ACCOUNT_AVAILABLE_PHONE_NUMBER_COUNTRY_AVAILABLE_PHONE_NUMBER_TOLL_FREEFields,
            }
          })
