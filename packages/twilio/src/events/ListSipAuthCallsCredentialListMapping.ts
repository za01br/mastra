    ...
    export const ListSipAuthCallsCredentialListMapping: EventHandler<TwilioIntegration> = ({
      eventKey,
      integrationInstance: { name, dataLayer, getApiClient, config },
      makeWebhookUrl,
    }) => ({
        id: `${name}-sync-API_V2010_ACCOUNT_SIP_SIP_DOMAIN_SIP_AUTH_SIP_AUTH_CALLS_SIP_AUTH_CALLS_CREDENTIAL_LIST_MAPPING-ListSipAuthCallsCredentialListMapping`,
        event: eventKey,
        executor: async ({ event, step }: any) => {
          ...
          // @ts-ignore
          const response = await proxy['/2010-04-01/Accounts/{AccountSid}/SIP/Domains/{DomainSid}/Auth/Calls/CredentialListMappings.json'].get({
          ...
          if (!response.ok) {
            ...
            console.log("error in fetching ListSipAuthCallsCredentialListMapping", JSON.stringify(error, null, 2));
            return
          }
