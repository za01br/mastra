    ...
    export const ListConference: EventHandler<TwilioIntegration> = ({
      ...
      makeWebhookUrl,
        ...
        executor: async ({ event, step }: any) => {
          ...
          const { DateCreated, DateCreated<, DateCreated>, DateUpdated, DateUpdated<, DateUpdated>, FriendlyName, Status, PageSize, Page, PageToken, AccountSid } = event.data;
          const proxy = await getApiClient({ referenceId })        

          // @ts-ignore
          const response = await proxy['/2010-04-01/Accounts/{AccountSid}/Conferences.json'].get({
            params: { AccountSid },
            query: { DateCreated, DateCreated<, DateCreated>, DateUpdated, DateUpdated<, DateUpdated>, FriendlyName, Status, PageSize, Page, PageToken },
          })
          ...
          const records = d?.['conferences']?.map((r) => {
            return {
              externalId: r.sid,
              record: r,
              entityType: API_V2010_ACCOUNT_CONFERENCEFields,
            } 
          })
