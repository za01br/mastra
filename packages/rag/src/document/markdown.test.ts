import { MarkdownTextSplitter, MarkdownHeaderTextSplitter } from './markdown';

describe('MarkdownTextSplitter', () => {
  let splitter: MarkdownTextSplitter;
...
describe('MarkdownHeaderTextSplitter', () => {
  ...
  beforeEach(() => {
    splitter = new MarkdownHeaderTextSplitter([
      ['#', 'Header 1'],
      ['##', 'Header 2'],
      ['###', 'Header 3'],
    ]);
  ...
  Final content.`;
    ...
    expect(section1?.pageContent).toContain('Section 1 content');
  });
  ...
  ## Next Section`;
    ...
    const docs = splitter.splitText(text);
    const codeDoc = docs.find(doc => doc.pageContent.includes('```python'));
    expect(codeDoc?.pageContent).toContain('print("Hello World")');
  });

  it('should respect returnEachLine option', () => {
    const splitterWithLines = new MarkdownHeaderTextSplitter([['#', 'Header 1']], true);
  ...
  it('should handle stripHeaders option', () => {
    const splitterWithHeaders = new MarkdownHeaderTextSplitter([['#', 'Header 1']], false, false);
