import { HTMLHeaderTextSplitter, HTMLSectionSplitter } from './html';

describe('HTMLHeaderTextSplitter', () => {
  let splitter: HTMLHeaderTextSplitter;

  beforeEach(() => {
    splitter = new HTMLHeaderTextSplitter([
      ['h1', 'Header 1'],
      ['h2', 'Header 2'],
      ['h3', 'Header 3'],
    ]);
  ...
  it('should handle nested content', async () => {
    ...
    expect(mainSection?.pageContent).toContain('Nested content');
    expect(mainSection?.pageContent).toContain('Deeply nested content');
  });

  it('should respect returnEachElement option', async () => {
    const splitterWithElements = new HTMLHeaderTextSplitter([['h1', 'Header 1']], true);
    ...
    const docs = await splitterWithElements.splitText(html);
...
describe('HTMLSectionSplitter', () => {
  let splitter: HTMLSectionSplitter;
