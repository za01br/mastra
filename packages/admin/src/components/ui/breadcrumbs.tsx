...
const BreadcrumbList = React.forwardRef<HTMLOListElement, React.ComponentPropsWithoutRef<'ol'>>(
  ({ className, ...props }, ref) => (
    <ol
      ref={ref}
      className={cn(
        'text-kp-el-6 flex items-center gap-2 break-words text-[calc(13_/_16_*_1rem)] font-medium',
        className,
      )}
      {...props}
    />
...
const BreadcrumbPage = React.forwardRef<HTMLSpanElement, React.ComponentPropsWithoutRef<'span'>>(
  ({ className, ...props }, ref) => (
    <span
      ...
      aria-disabled="true"
      aria-current="page"
      className={cn('text-kp-el-5 text-sm font-medium', className)}
      {...props}
    />
  ),
);
...
const BreadcrumbSeparator = ({ children, className, ...props }: React.ComponentProps<'li'>) => (
  <li role="presentation" aria-hidden="true" className={cn(className)} {...props}>
    {children ?? <Icon name="chevron-down" className="text-kp-el-3 !h-[0.625rem] !w-[0.625rem] -rotate-90" />}
  </li>
);
...
export default function Breadcrumb({ items, icon, className, pageClassName }: BreadcrumbProps) {
  return (
    <div className="flex items-center gap-4">
      {icon && (
        <Icon name={icon} width={16} height={16} className={cn('w-[0.625rem] text-kp-el-3 h-[0.625rem]', className)} />
      )}
      <BreadcrumbPrimitive>
        <BreadcrumbList>
                ...
                <BreadcrumbItemPrimitive key={item.href}>
                  {item.isCurrent ? (
                    ...
                    <BreadcrumbLink asChild>
                      <Link href={item.href} className="text-kp-el-3">
                        {item.label}
                      </Link>
                    </BreadcrumbLink>
                  )}
