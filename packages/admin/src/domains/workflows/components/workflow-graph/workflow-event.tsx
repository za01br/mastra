      ...
      <>
        <button
          ...
          className={cn(
            ...
            {
              'border-kp-border-5': selectedBlock?.type === 'trigger',
            },
          )}
          title="New action"
        >
          <div className={cn('flex gap-[6px] rounded-sm bg-[#2C2C2C] p-2')}>
            ...
            <Text className="text-kp-el-4" size="xs" weight="medium">
              Trigger
            </Text>
          </div>
  ...
  return (
    <TooltipProvider>
      <button
        type="button"
        onClick={handleTriggerClick}
        className={cn(blockStyles.default, blockStyles.states, 'bg-kp-bg-8 min-w-[17rem] rounded-[0.3125rem]', {
          'border-kp-border-5': selectedBlock?.block?.id === trigger?.id,
        })}
      >
        <div className={cn(blockStyles.header)}>
          ...
          {attemptedPublish && !isTriggerValid && (
            <Tooltip>
              <TooltipTrigger>
                <div className={cn('border-kp-border-2 bg-kp-bg-9 rounded-sm border-[0.4px] border-solid p-1', {})}>
                  <Icon name="warning-square" className="text-kp-el-warning" />
                </div>
              </TooltipTrigger>
              <TooltipPortal>
