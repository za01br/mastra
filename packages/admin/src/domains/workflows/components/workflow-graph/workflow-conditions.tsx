...
export const blockStyles = {
  default: 'border-[0.5px] p-[0.38rem] flex gap-1 border-solid rounded-md relative border-kpl-border-1',
  states: 'hover:border-kpl-border-5 focus:border-kpl-border-5',
  header: ' text-[13px] flex gap-3 items-center',
  details: 'bg-kpl-bg-3 rounded-b-md text-[10px] text-left text-neutral-300',
};
  ...
  return (
    <>
      {isFirstItem || isLastItem ? (
        <div
          role="presentation"
          className={cn('border-kpl-el-1 relative -mt-3 mb-[36px] h-[8px] w-1/2 border-t bg-transparent', {
        ...
        >
          <div
            role="presentation"
            className={cn('from-kpl-el-1 to-kpl-bg-1 absolute top-[7px] h-[36px] w-[1px] bg-gradient-to-b', {
        ...
        <>
          <div
            role="presentation"
            className={cn('border-kpl-el-1 relative -mt-3 h-[1px] border-t bg-transparent', {
              'w-full': noOfActions > 1,
            })}
          />
          <div
            role="presentation"
            className={cn('from-kpl-el-1 to-kpl-bg-1 relative h-[43px] w-[1px] bg-gradient-to-b', {})}
          />
        </>
      )}
      <ContextMenu>
        <ContextMenuTrigger aria-label="context-menu" className="relative">
          <button
            className={cn(
              ...
              selectedBlock?.block?.id === conditionGroup?.id ? 'border-kpl-border-5' : '',
          ...
          >
            {conditionGroup?.isDefault ? (
              <>
                <div className="text-kpl-el-3 flex items-center gap-[0.63rem]">
                  <div className="bg-kpl-bg-9 border-kpl-border-2 flex h-fit items-center justify-center rounded-[0.2rem] border-[0.4px] px-[0.34rem] py-[0.32rem] align-middle">
                    <Icon name="rule" className="text-kpl-el-3" />
                  </div>
              ...
              <>
                <Text
                  size={'xs'}
                  weight={'medium'}
                  className="bg-kpl-bg-9 border-kpl-border-2 flex h-fit items-center justify-center rounded-[0.2rem] border-[0.4px] px-[0.46rem] py-[0.32rem] align-middle"
                >
                  {pathAlphabet[path]}
                </Text>
                ...
                {isConditionValid(conditionGroup) ? (
                          ...
                          <div key={index} className="flex items-stretch gap-1">
                            {condition.conj && condition.field ? (
                              <Text
                                size={'xs'}
                                weight="medium"
                                className="text-kpl-el-3 bg-kpl-bg-11 my-auto block rounded-[0.125rem] px-2 py-1 text-[10px]"
                              >
                            ...
                            {condition.field ? (
                              <div
                                className={cn(
                                  'bg-kpl-bg-3 border-kpl-border-2 flex w-full items-center rounded-[0.25rem] border-[0.5px]',
                                )}
                              >
                                {condition.field ? (
                                  <Text
                                    size={'xs'}
                                    className="border-r-kpl-border-2 text-kpl-el-3 border-r-[0.5px] px-[0.38rem] py-[0.19rem]"
                                  >
                                ...
                                {condition.operator ? (
                                  <Text
                                    size={'xs'}
                                    className="border-r-kpl-border-2 text-kpl-el-3 border-r-[0.5px] px-[0.38rem] py-[0.19rem] lowercase"
                                  >
                                    {condition.operator}
                                  </Text>
                                ) : null}
                                ...
                                {condition.value ? (
                                  <Text size={'xs'} className="text-kpl-el-3 px-[0.38rem] py-[0.19rem]">
                  ...
                  <Text className="text-kpl-el-3 flex gap-1 px-[0.38rem]">
                    <Icon name="rule" className="text-kpl-el-3" />
                    <Text size={'xs'} className="text-kpl-el-3">
                      Path condition
                    </Text>
