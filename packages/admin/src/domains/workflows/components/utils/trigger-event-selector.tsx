import type { UpdateTrigger, RefinedIntegrationEvent } from '@mastra/core';
...
export function TriggerEventSelector({
  ...
  return (
    <Button
      ...
      variant="ghost"
      className="bg-mastra-bg-6 border-mastra-border-1 h-9 justify-start gap-3 border-[0.5px] py-0 pl-0 pr-3 text-[#a9a9a9] opacity-70 transition-colors hover:opacity-100 w-full"
    >
      <div className="border-mastra-border-1 flex h-full w-9 items-center justify-center border-r-[0.3px]">
        {/* <FrameworkIcon icon={""} className="text-base text-current" /> */}
      </div>
      <span className="text-mastra-el-6 text-xs font-[500]">{label}</span>
