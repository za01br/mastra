...
const Tools = () => {
  ...
  const { devTools, isLoading: isLoadingDevTools } = useDevTools();
  ...
  useEffect(() => {
    ...
    setSelectedAgentId(Object.keys(agents)[0]);
  }, [isLoadingAgents]);
      ...
      <div className="w-full h-full grid grid-cols-[300px_1fr]  ">
        <div className="w-full h-full border-r-[0.5px] border-mastra-border-1 py-6 px-4">
          <ul className=" flex flex-col gap-4">
            ...
            {Object.keys(devTools).length > 0 && (
              <li
                className={cn(
                  'px-2 py-2 rounded-md hover:bg-mastra-bg-4/80 transition-colors cursor-pointer',
                  isDevTools && 'bg-mastra-bg-4/80',
                )}
                onClick={handleSelectDevTools}
              >
                <div className="flex gap-2 items-center">
                  <svg
                    xmlns="http://www.w3.org/2000/svg"
                  ...
                  <p className="text-sm text-mastra-el-6">Dev Tools</p>
                </div>
              </li>
            )}
          </ul>
        ...
        <div className="flex flex-col gap-4 py-6 px-4">
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-2">
            {isLoadingAgent ? (
              ...
              Object.entries(devTools).map(([name, tool], index) => (
                <div
                  onClick={() => {
                    navigate(`/tools/dev-tools/${tool.id}`);
                  }}
                  key={index}
