...
import { useDevTool } from '@/hooks/use-dev-tools';
import { useExecuteDevTool } from '@/hooks/use-execute-dev-tool';

const DevTool = () => {
  const { toolId } = useParams();
  const { executeDevTool, isExecuting } = useExecuteDevTool();
  const [result, setResult] = useState<any>(null);
  const { tool, isLoading } = useDevTool(toolId!);

  const handleExecuteTool = async (data: any) => {
    if (!tool) return;

    const result = await executeDevTool({
      toolId: tool.id,
      input: data,
    });
    setResult(result);
