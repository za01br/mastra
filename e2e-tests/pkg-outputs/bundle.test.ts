...
describe.for(allPackages.map(pkg => [relative(join(__dirname, '..'), dirname(pkg)), pkg]))(
  '%s',
  async ([pkgName, packagePath]) => {
    ...
    describe.concurrent.for(imports.map(x => [x]))('%s', async ([importPath]) => {
      ...
      it('should use .cjs and .d.cts extensions when using require', async () => {
        if (importPath === './package.json') {
          return;
        }
